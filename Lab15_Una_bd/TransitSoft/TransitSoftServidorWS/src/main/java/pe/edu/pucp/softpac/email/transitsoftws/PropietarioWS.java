...
public class PropietarioWS {
    ...
    @GET
    @Path("{id}")
    public Response obtenerPropietarioVehiculo(@PathParam("id") Integer vehiculoId){
        PropietarioDTO propietario = this.propietarioBO.obtenerPropietarioVehiculo(vehiculoId);
        if (propietario==null){
