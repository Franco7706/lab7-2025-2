...
public abstract class DAOImplBase {
    ...
    protected void iniciarTransaccion(MotorDeBaseDeDatos motor) throws SQLException {
    ...
    protected Integer insertar(MotorDeBaseDeDatos motor) {
        return this.ejecuta_DML(Tipo_Operacion.INSERTAR,motor);
    }

    protected Integer modificar(MotorDeBaseDeDatos motor) {
        return this.ejecuta_DML(Tipo_Operacion.MODIFICAR,motor);
    }

    protected Integer eliminar(MotorDeBaseDeDatos motor) {
        return this.ejecuta_DML(Tipo_Operacion.ELIMINAR,motor);
    }

    private Integer ejecuta_DML(Tipo_Operacion tipo_operacion,MotorDeBaseDeDatos motor) {
        Integer resultado = 0;
        try {
            this.iniciarTransaccion(motor);
            String sql = null;
            ...
            if (this.retornarLlavePrimaria && tipo_operacion == Tipo_Operacion.INSERTAR) {
                resultado = this.retornarUltimoAutoGenerado(motor);
            }
            if (tipo_operacion == Tipo_Operacion.INSERTAR) {
                this.insertarEnTablaIntermedia(resultado,motor);
            }
    ...
    public Integer retornarUltimoAutoGenerado(MotorDeBaseDeDatos motor) {
        Integer resultado = null;
        try {
            String sql;
            if(motor.equals(MotorDeBaseDeDatos.MYSQL))
                sql = DBManager.getInstanceMySQL().retornarSQLParaUltimoAutoGenerado();
            else
                sql = DBManager.getInstanceMSSQL().retornarSQLParaUltimoAutoGenerado();
            this.statement = this.conexion.prepareCall(sql);
            this.resultSet = this.statement.executeQuery();
    ...
    public void obtenerPorId(MotorDeBaseDeDatos motor) {
        try {
            this.abrirConexion(motor);
